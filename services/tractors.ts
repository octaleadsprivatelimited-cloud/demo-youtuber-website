...
export async function listTractors(filters: TractorFilters = {}, cursor?: DocumentSnapshot | number | null): Promise<TractorPage> { const pageSize = Math.min(filters.pageSize ?? 12, 24); if (!db) {
    const saved = isLocalDemo ? await locals() : [];
    let items = saved.length ? saved : demoTractors;
...
export async function searchTractorSuggestions(prefix: string) { if (!db) {
    const saved = isLocalDemo ? await locals() : [];
    return (saved.length ? saved : demoTractors).filter(x => x.name.toLowerCase().includes(prefix.toLowerCase())).slice(0, 6); } const snap = await getDocs(query(collection(db, 'tractors'), where('status', '==', 'published'), where('searchPrefixes', 'array-contains', prefix.trim().toLowerCase().slice(0, 30)), orderBy('popularityScore', 'desc'), limit(6))); return snap.docs.map(d => normalizeTractor({ id: d.id, ...d.data() })); }
export async function getTractorBySlugs(brandSlug: string, modelSlug: string) { if (!db) {
    const saved = isLocalDemo ? await locals() : [];
    return (saved.length ? saved : demoTractors).find(x => x.brandSlug === brandSlug && x.slug === modelSlug) ?? null; } const snap = await getDocs(query(collection(db, 'tractors'), where('brandSlug', '==', brandSlug), where('slug', '==', modelSlug), where('status', '==', 'published'), limit(1))); const d = snap.docs[0]; return d ? normalizeTractor({ id: d.id, ...d.data() }) : null; }
export async function listBrands(): Promise<Brand[]> { if (!db) {
    if (isLocalDemo) {
        const saved = await published<Record<string, unknown> & { id: string }>('brands');
