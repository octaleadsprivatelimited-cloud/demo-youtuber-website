...
export async function searchTractorSuggestions(prefix: string) { if (!db) {
    return demoTractors.filter(x => x.name.toLowerCase().includes(prefix.toLowerCase())).slice(0, 6); } const snap = await getDocs(query(collection(db, 'tractors'), where('status', '==', 'published'), where('searchPrefixes', 'array-contains', prefix.trim().toLowerCase().slice(0, 30)), orderBy('popularityScore', 'desc'), limit(6))); return snap.docs.map(d => normalizeTractor({ id: d.id, ...d.data() })); }
export async function getTractorBySlugs(brandSlug: string, modelSlug: string) { if (!db) {
    return demoTractors.find(x => x.brandSlug === brandSlug && x.slug === modelSlug) ?? null; } const snap = await getDocs(query(collection(db, 'tractors'), where('brandSlug', '==', brandSlug), where('slug', '==', modelSlug), where('status', '==', 'published'), limit(1))); const d = snap.docs[0]; return d ? normalizeTractor({ id: d.id, ...d.data() }) : null; }
