...
function normalize(raw: Record<string, unknown>): Tractor { const brand = String(raw.brandName ?? raw.brand ?? 'Unbranded'); const model = String(raw.modelName ?? raw.model ?? raw.title ?? 'Untitled'); return { ...raw, id: String(raw.id), name: String(raw.displayName ?? raw.name ?? `${brand} ${model}`), brandName: brand, brandId: String(raw.brandId ?? brand.toLowerCase().replaceAll(' ', '-')), brandSlug: String(raw.brandSlug ?? brand.toLowerCase().replaceAll(' ', '-')), model, slug: String(raw.slug ?? model.toLowerCase().replaceAll(' ', '-')), hp: Number(raw.hp ?? raw.horsepower ?? 0), minPrice: Number(raw.minPrice ?? raw.priceMin ?? raw.price ?? 0), maxPrice: Number(raw.maxPrice ?? raw.priceMax ?? raw.price ?? 0), transmission: String(raw.transmission ?? 'Not specified'), image: String(raw.thumbnail ?? raw.image ?? ''), status: 'published' } as Tractor; }
async function locals() { return (await published<Record<string, unknown> & {
    id: string;
}>('tractors')).map(normalize); }
export async function listTractors(filters: TractorFilters = {}, cursor?: DocumentSnapshot | null): Promise<TractorPage> { const pageSize = Math.min(filters.pageSize ?? 12, 24); if (isLocalDemo && !db) {
    ...
    return { items: items.slice(0, pageSize), cursor: null, hasMore: items.length > pageSize };
} if (!db)
    ...
    constraints.push(where('brandId', '==', filters.brandId)); if (filters.driveType)
    ...
    constraints.push(orderBy('minPrice')); constraints.push(orderBy('popularityScore', 'desc'), orderBy(documentId()), limit(pageSize + 1)); if (cursor)
    constraints.splice(constraints.length - 1, 0, startAfter(cursor)); const snapshot = await getDocs(query(collection(db, 'tractors'), ...constraints)); const docs = snapshot.docs.slice(0, pageSize); return { items: docs.map(d => normalize({ id: d.id, ...d.data() })), cursor: docs.at(-1) ?? null, hasMore: snapshot.docs.length > pageSize }; }
export async function searchTractorSuggestions(prefix: string) { if (isLocalDemo && !db)
    ...
    return []; const snap = await getDocs(query(collection(db, 'tractors'), where('status', '==', 'published'), where('searchPrefixes', 'array-contains', prefix.trim().toLowerCase().slice(0, 30)), orderBy('popularityScore', 'desc'), limit(6))); return snap.docs.map(d => normalize({ id: d.id, ...d.data() })); }
export async function getTractorBySlugs(brandSlug: string, modelSlug: string) { if (isLocalDemo && !db)
    ...
    return null; const snap = await getDocs(query(collection(db, 'tractors'), where('brandSlug', '==', brandSlug), where('slug', '==', modelSlug), where('status', '==', 'published'), limit(1))); const d = snap.docs[0]; return d ? normalize({ id: d.id, ...d.data() }) : null; }
