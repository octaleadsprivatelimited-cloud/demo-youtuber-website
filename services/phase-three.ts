...
export async function isFavourite(userId: string, itemType: FavouriteRecord['itemType'], itemId: string) {
    if (isLocalDemo && !db)
        return (await readLocal<FavouriteRecord>('favorites')).some(item => item.userId === userId && item.itemType === itemType && item.itemId === itemId);
...
export async function toggleFavourite(record: Omit<FavouriteRecord, 'id' | 'createdAt'>) {
    if (isLocalDemo && !db) {
        const items = (await readLocal<FavouriteRecord>('favorites'));
        const found = items.find(item => item.userId === record.userId && item.itemType === record.itemType && item.itemId === record.itemId);
        (await writeLocal('favorites', found ? items.filter(item => item.id !== found.id) : [{ ...record, id: `demo-${Date.now()}` }, ...items]));
        return !found;
    }
...
export async function listFavourites(userId: string): Promise<FavouriteRecord[]> {
    if (isLocalDemo && !db)
        return (await readLocal<FavouriteRecord>('favorites')).filter(item => item.userId === userId);
