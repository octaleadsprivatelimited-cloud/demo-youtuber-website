...
export async function listLeads(filters: {
    ...
    return (await demos()).filter(x => (!filters.status || x.status === filters.status) && (!filters.source || x.source === filters.source)); const clauses: QueryConstraint[] = [orderBy('createdAt', 'desc'), limit(100)]; if (filters.status)
    clauses.unshift(where('status', '==', filters.status)); if (filters.source)
    clauses.unshift(where('source', '==', filters.source)); const snap = await getDocs(query(collection(database(), 'leads'), ...clauses)); return snap.docs.map(x => ({ id: x.id, ...x.data() }) as Lead); }
export async function updateLeadStatus(id: string, status: LeadStatus, assignedTo?: string) { if (isLocalDemo && !db) {
    (await save((await demos()).map(x => x.id === id ? { ...x, status, assignedTo: assignedTo ?? x.assignedTo } : x)));
    return;
} await updateDoc(doc(database(), 'leads', id), { status, ...(assignedTo !== undefined ? { assignedTo } : {}), updatedAt: serverTimestamp() }); }
export async function updateLeadNotes(id: string, notes: string) { if (isLocalDemo && !db) {
    (await save((await demos()).map(x => x.id === id ? { ...x, notes, updatedAt: new Date().toISOString() } : x)));
    return;
} await updateDoc(doc(database(), 'leads', id), { notes, updatedAt: serverTimestamp() }); }
