...
export async function GET(_request: Request, {params}: {params: Promise<{id: string}>}) {
  ...
  try {
    ...
    const result = await fetch(`https://firestore.googleapis.com/v1/projects/${encodeURIComponent(project)}/databases/(default)/documents/media/${id}`, {cache: 'no-store'});
    ...
    return new Response(Uint8Array.from(decoded, character => character.charCodeAt(0)), {headers: {
      ...
      'X-Content-Type-Options': 'nosniff', 'Cache-Control': 'no-store, max-age=0',
    }});
