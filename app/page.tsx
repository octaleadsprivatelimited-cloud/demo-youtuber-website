...
import { HomeIntroduction, HomeBrands, HomeCompare, HomeArticles, HomeVideos, HomePartners } from '@/components/HomepageSections';
...
export default function Home() {
  ...
  const { items: tractors, loading: tractorsLoading, error: tractorsError, retry: retryTractors } = useTractorCatalog();
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => setHeroIndex(index => (index + 1) % heroSlides.length), 3000);
  ...
  useEffect(() => {
    ...
    const prepare = () => {
      root.querySelectorAll<HTMLElement>(':scope > section').forEach(section => {
        ...
        section.querySelectorAll<HTMLElement>('.tractor-card,.home-brand-card,.home-compare-card,.home-article-card,.home-video-card,.cms-home-card').forEach((item,index) => {
  ...
  return <div className="reference-home"><HomepageHeader/><main>{resolveHomepageSections(sectionRecords).map(section => <HomeSection key={section.key}>{renderSection(section.key, section.title)}</HomeSection>)}</main><div className="home-v2 home-footer"><SiteFooter/></div></div>;
}
