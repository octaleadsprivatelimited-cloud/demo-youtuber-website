...
export function LeadEditDialog({ lead, onClose, onSave }: { lead: Lead; onClose: () => void; onSave: (lead: Lead) => void }) {
  ...
  return <AdminDialog title="Edit lead details" busy={saving} onClose={onClose}><form className="cms-form" onSubmit={submit}>
    <LocalizedElement as="p" className="cms-help">Only Name and Phone are required. Updating contact details keeps the enquiry source, status and internal notes.</LocalizedElement>
    <LocalizedElement as="div" className="cms-fields">{fields.map(field => <LocalizedElement as="label" key={field.key} className={field.key === 'message' ? 'cms-wide' : ''}>{field.label}{'required' in field ? ' *' : ' (optional)'}
      {field.key === 'message' ? <LocalizedElement as="textarea" rows={4} disabled={saving} value={form[field.key]} onChange={event => setForm(current => ({ ...current, [field.key]: event.target.value }))}/> : <LocalizedElement as="input" required={'required' in field} disabled={saving} type={field.key === 'email' ? 'email' : field.key === 'phone' ? 'tel' : 'text'} value={form[field.key]} onChange={event => setForm(current => ({ ...current, [field.key]: event.target.value }))}/>}
    </LocalizedElement>)}</LocalizedElement>
    {error && <LocalizedElement as="p" className="cms-error" role="alert">{error}</LocalizedElement>}
    <LocalizedElement as="div" className="cms-form-footer"><LocalizedElement as="button" type="button" disabled={saving} onClick={onClose}>Cancel</LocalizedElement><LocalizedElement as="button" className="cms-primary" disabled={saving}>{saving ? 'Saving…' : 'Save changes'}</LocalizedElement></LocalizedElement>
  </form></AdminDialog>;
}
