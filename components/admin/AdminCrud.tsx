  ...
  return <LocalizedElement as="div" className="cms-admin">
    ...
    <section className="cms-panel">
      ...
      {hero ? <LocalizedElement as="div" className="cms-slide-grid">{visible.map(item => <article key={item.id} className="cms-slide">
        ...
        <LocalizedElement as="div" className="cms-slide-details"><LocalizedElement as="h2">{recordName(item)}</LocalizedElement><LocalizedElement as="small">{String(item.status ?? 'published')}</LocalizedElement><LocalizedElement as="p">{item.image ? 'Saved image' : 'No image · background colour only'}</LocalizedElement>
        ...
        <LocalizedElement as="div"><LocalizedElement as="strong">{recordName(item)}</LocalizedElement><LocalizedElement as="small">{String(item.slug ?? item.path ?? item.key ?? '')}</LocalizedElement><LocalizedElement as="small">{String(item.status ?? 'published')}</LocalizedElement>{section.collection==='expertReviews'&&<><LocalizedElement as="small">{String(item.status || 'draft')}</LocalizedElement>{item.status==='published'&&<LocalizedElement as="a" href={'/reviews/'+item.slug} target="_blank" rel="noreferrer">View review ↗</LocalizedElement>}</>}{section.moderation&&<LocalizedElement as="small">{String(item.status??'pending')}</LocalizedElement>}</LocalizedElement>
    ...
    {open && <AdminDialog title={(section.readOnly?'View ':editing ? 'Edit ' : 'Add ') + (hero ? 'hero slide' : section.label.toLowerCase())} busy={busy} onClose={() => setOpen(false)}>
      <form className="cms-form" onSubmit={submit}>
        ...
        {section.collection === 'tractors' ? <TractorFields form={form} sources={sources} disabled={busy||Boolean(section.readOnly)} onChange={change} onImport={values=>setForm(current=>({...current,...values}))} onBusy={setUploading} onError={setFormError}/> : <LocalizedElement as="div" className="cms-fields">{section.fields.map(field => field.type === 'image' || (section.collection === 'settings' && field.key === 'value' && form.key === 'logo') ?
          ...
          <LocalizedElement as="label" key={field.key} className={field.type === 'textarea' ? 'cms-wide' : ''}>{field.label}{field.required ? <LocalizedElement as="span" aria-hidden="true"> *</LocalizedElement> : <LocalizedElement as="small" className="cms-optional"> (optional)</LocalizedElement>}
            {field.type === 'select' ? <LocalizedElement as="select" disabled={busy||Boolean(section.readOnly)} required={field.required} value={String(form[field.key]??'')} onChange={event=>{ change(field.key,event.target.value); if(section.collection==='settings' && field.key==='key') change('value',''); }}>
              ...
              </LocalizedElement> : ['textarea','lines'].includes(field.type ?? '') ? <LocalizedElement as="textarea" required={field.required} disabled={busy||Boolean(section.readOnly)} rows={field.key === 'content' ? 8 : 3} value={Array.isArray(form[field.key]) ? (form[field.key] as string[]).join('\n') : String(form[field.key] ?? '')} onChange={event => change(field.key, event.target.value)}/> :
