...
export function TractorFields({ form, sources, disabled, onChange, onImport, onBusy, onError }: {
  ...
  function fieldInput(field: AdminField | TractorSpecField) {
    ...
    return <LocalizedElement as="label" key={field.key} htmlFor={id} className={['textarea', 'lines'].includes(field.type ?? '') ? 'cms-wide' : ''}>{label}{base.required ? ' *' : ''}
      {field.type === 'boolean' ? <LocalizedElement as="input" id={id} type="checkbox" checked={value === true} disabled={disabled} onChange={event => onChange(field.key, event.target.checked)}/> : field.type === 'select' ? <LocalizedElement as="select" id={id} required={base.required} disabled={disabled} value={stringValue} onChange={event => onChange(field.key, event.target.value)}><LocalizedElement as="option" value="">Not specified</LocalizedElement>{choices?.map(option => <LocalizedElement as="option" key={option.value} value={option.value}>{option.label}</LocalizedElement>)}</LocalizedElement> : ['textarea', 'lines'].includes(field.type ?? '') ? <LocalizedElement as="textarea" id={id} rows={field.type === 'lines' ? 4 : 3} disabled={disabled} maxLength={12000} value={stringValue} onChange={event => onChange(field.key, event.target.value)} aria-describedby={spec.help ? id + '-help' : undefined} /> : <LocalizedElement as="input" id={id} required={base.required} disabled={disabled} type={field.type === 'number' ? 'number' : field.key === 'specificationSourceUrl' ? 'url' : 'text'} min={spec.min ?? (field.type === 'number' ? 0 : undefined)} step={spec.integer ? 1 : field.type === 'number' ? 'any' : undefined} maxLength={600} value={stringValue} onChange={event => onChange(field.key, field.type === 'number' && event.target.value !== '' ? Number(event.target.value) : event.target.value)} aria-describedby={spec.help ? id + '-help' : undefined} />}
      {spec.help && <LocalizedElement as="small" id={id + '-help'} className="tractor-field-help">{spec.help}</LocalizedElement>}
    </LocalizedElement>;
  }
  return <LocalizedElement as="div" className="tractor-spec-editor" onInvalidCapture={event => { const details = (event.target as HTMLElement).closest('details'); if (details) details.open = true; }}>
    ...
    <section className="tractor-csv-import" aria-label="Upload tractor specifications"><LocalizedElement as="div"><LocalizedElement as="h3">Upload specifications</LocalizedElement><LocalizedElement as="p">Use a two-column CSV for this tractor. Preview it before applying. Blank values are ignored.</LocalizedElement></LocalizedElement><LocalizedElement as="div" className="tractor-import-actions"><LocalizedElement as="a" download="tractor-specifications-template.csv" href={'data:text/csv;charset=utf-8,' + encodeURIComponent(tractorSpecificationTemplate())}>Download CSV template ↓</LocalizedElement><LocalizedElement as="label">Choose CSV<LocalizedElement as="input" type="file" accept=".csv,text/csv" disabled={disabled} onChange={event => { void read(event.target.files?.[0]); event.target.value = ''; }} /></LocalizedElement></LocalizedElement>
      ...
      {preview && <LocalizedElement as="div" className="tractor-import-preview"><LocalizedElement as="h4">{Object.keys(preview.values).length} specifications ready to apply</LocalizedElement><dl>{Object.entries(preview.values).map(([key, value]) => <LocalizedElement as="div" key={key}><dt>{tractorSpecFields.find(field => field.key === key)?.label}</dt><dd>{Array.isArray(value) ? value.join(' · ') : String(value)}</dd></LocalizedElement>)}</dl><LocalizedElement as="p">This replaces only the listed fields in the form. Other fields stay unchanged.{preview.skipped ? ' ' + preview.skipped + ' blank values skipped.' : ''}</LocalizedElement><LocalizedElement as="div" className="tractor-import-actions"><LocalizedElement as="button" type="button" disabled={disabled} onClick={() => setPreview(null)}>Discard preview</LocalizedElement><LocalizedElement as="button" className="cms-primary" type="button" disabled={disabled} onClick={() => { onImport(preview.values); setNotice('Specifications applied to the form. Review them, then save the tractor.'); setPreview(null); }}>Apply to form</LocalizedElement></LocalizedElement></LocalizedElement>}
    </section>
    {tractorSpecGroups.map((group, index) => <details className="tractor-spec-group" key={group.key} open={index === 0}><LocalizedElement as="summary"><LocalizedElement as="span">{group.title}</LocalizedElement><LocalizedElement as="small">{group.fields.filter(field => { const value = form[field.key]; return value !== undefined && value !== '' && (!Array.isArray(value) || value.length); }).length}/{group.fields.length} filled</LocalizedElement></LocalizedElement><LocalizedElement as="p">{group.description}</LocalizedElement><LocalizedElement as="div" className="cms-fields">{group.fields.map(fieldInput)}</LocalizedElement></details>)}
    <LocalizedElement as="p" className="tractor-spec-references">Specification checklist based on official manufacturer pages: {tractorSpecificationSources.map((source, index) => <LocalizedElement as="span" key={source.url}>{index ? ' · ' : ''}<LocalizedElement as="a" href={source.url} target="_blank" rel="noreferrer">{source.title.split(' ')[0]}</LocalizedElement></LocalizedElement>)}. Enter the values for your exact model and variant; these links do not fill values automatically.</LocalizedElement>
  </LocalizedElement>;
}
