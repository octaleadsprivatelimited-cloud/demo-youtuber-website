...
async function requireTargetAccess(callerUid:string,callerRole:string,uid:string){
  ...
  const profile=await getFirestore().doc('admins/'+uid).get();
  const targetRole=profile.data()?.role??target.customClaims?.role;
  if((['Admin','Super Admin'].includes(targetRole)||target.customClaims?.admin===true)&&callerRole!=='Super Admin')throw new HttpsError('permission-denied','Only a Super Admin can change administrator accounts.');
  return target;
}
